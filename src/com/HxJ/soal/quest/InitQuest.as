package com.HxJ.soal.quest {
	
	import flash.display.MovieClip;
	import com.HxJ.utils.XMLLoader;
	...
	public class InitQuest {
		...
		private function xmlInit( xml:XML ): void {
			...
			for ( var i:int = 0, ii:int = 0, len:int = xml.soal.length(); i < len; i++ ) {
				
				if( xml.soal[ i ].@aktif == 'ya' ) {
					
					Q.Q[ ii ] = xmlQuest( xml.soal[ i ], {} );
					Q.O[ ii ] = ii++;
